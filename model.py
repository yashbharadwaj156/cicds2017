...
class NodeClassificator(nn.Module):
    def __init__(self, dataset, num_classes, num_convs: int = 8, hid: int = 128, dropout: float = 0.5,
                 alpha: float = 0.5, theta: float = 0.7, bigru_hidden_size: int = 64, bigru_layers: int = 2, timestamp: int = 5):
        ...
        self.fc1 = nn.Linear(bigru_hidden_size * 2, hid)  # BiGRU is bidirectional, so hidden_size * 2
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # Reshape for BiGRU: (batch_size, sequence_length, feature_size)
        batch_size = x.size(0)
        ...
        x, _ = self.bigru(x)  # BiGRU output

        # Take the last hidden state of the sequence
        x = x[:, -1, :]  # Shape: (batch_size // timestamp, bigru_hidden_size * 2)
